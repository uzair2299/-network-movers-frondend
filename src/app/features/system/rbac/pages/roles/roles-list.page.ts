...
export class RolesListPage implements OnInit, OnDestroy {
  ...
  loadRoles(): void {
    this.isLoading = true;
    this.error = null;
    this.rbacService.getRoles(this.currentPage, this.pageSize, this.sort, this.searchQuery)
  ...
  createNewRole(): void {
    ...
    dialogRef.afterClosed().subscribe((result: Role | null) => {
      if (result) {
        this.isLoading = true;
        this.rbacService.createRole(result).subscribe({
          next: () => {
          ...
          error: (err) => {
            ...
            this.toastService.showError('Failed to create role.', 'Error');
  ...
  viewRole(role: Role): void {
    this.isLoading = true;
    this.rbacService.getRoleById(role.id).pipe(
  ...
  editRole(role: Role): void {
    ...
    dialogRef.afterClosed().subscribe((result: Role | null) => {
      if (result) {
        this.isLoading = true;
        this.rbacService.updateRole(role.id, result).subscribe({
          next: () => {
          ...
          error: (err) => {
            ...
            this.toastService.showError('Failed to update role.', 'Error');
            this.isLoading = false;
          }
        });
      }
